...
export function PublicView({
  ...
  const centrosFiltrados = useMemo(() => {
    ...
    return centros
      .filter((c) => !texto || c.nombre.toLowerCase().includes(texto) || c.direccion.toLowerCase().includes(texto) || c.zona?.toLowerCase().includes(texto))
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-6 pb-16">
      <header className="mb-6">
        <h1 className="text-2xl font-bold text-ink">Centros de acopio</h1>
      ...
      <div className="mb-3 flex flex-col gap-2 sm:flex-row">
        ...
        <select
          ...
          className="sm:w-44 rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-1 focus:ring-ring"
        >
      ...
      {centrosFiltrados.length === 0 && (
        <p className="rounded-[14px] border border-line bg-surface p-6 text-center text-sm text-muted">
  ...
  return (
    <article className="rounded-[14px] border border-line bg-surface p-4 sm:p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-ink">{centro.nombre}</h2>
          <p className="mt-1 flex items-start gap-1.5 text-sm text-muted">
            <MapPin className="mt-0.5 h-4 w-4 shrink-0" />
            <span>
              {centro.direccion}
              {ubicacion && <span className="ml-1 font-medium text-ink">· {ubicacion}</span>}
            </span>
          </p>
          {centro.horario && <p className="mt-0.5 text-sm text-muted">Horario: {centro.horario}</p>}
        </div>
        <a
          href={`https://wa.me/?text=${mensajeWhatsapp}`}
          target="_blank"
          rel="noreferrer"
          className="flex shrink-0 items-center gap-1 rounded-full border border-line px-3 py-1.5 text-xs font-medium text-ink hover:bg-surface2"
        >
      ...
      <a
        ...
        className="mt-3 inline-flex items-center gap-1.5 rounded-[10px] bg-cubierto-bg px-3 py-1.5 text-sm font-medium text-cubierto"
      >
        <Phone className="h-4 w-4" /> {centro.contacto}
      </a>

      {porCategoria.length === 0 ? (
        <p className="mt-4 text-sm text-muted">Sin necesidades pendientes en este momento.</p>
      ) : (
        <div className="mt-4 flex flex-col gap-3">
          {porCategoria.map((cat) => (
            <div key={cat.value}>
              <h3 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-muted">{cat.label}</h3>
              <ul className="flex flex-col gap-1.5">
                {cat.items.map((i) => (
                  <li
                    key={i.id}
                    className="flex flex-wrap items-center justify-between gap-2 rounded-[10px] bg-surface2 px-3 py-2 text-sm"
                  >
                    <span className="text-ink">
                      {i.nombre}
                      {i.unidad && (
                        <span className="text-muted"> — {i.cantidad_cubierta}/{i.cantidad_necesaria} {i.unidad}</span>
                      )}
                    </span>
                    <span className="flex items-center gap-2">
                      <span className="flex items-center gap-1 text-xs text-muted" title={formatoHoraVenezuela(i.created_at)}>
